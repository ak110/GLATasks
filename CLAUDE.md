...
- 本リポジトリで使用するライブラリのAPI・設定・移行手順を参照する場合は `context7` MCPを優先する
- 対象ライブラリ例はSvelteKit / Svelte 5 (runes) / Tailwind CSS v4 / tRPC v11など
- そのほかDrizzle ORM / TanStack Query / Vitest 4 / Vite 8 / TypeScript 6なども対象
- 具体的なツールは `mcp__plugin_context7_context7__resolve-library-id` → `mcp__plugin_context7_context7__query-docs` の順で呼び出す
- 本リポジトリはClaudeの学習データより新しいメジャーバージョン（Svelte 5、Tailwind v4、tRPC v11、Vite 8など）に追従するため、知識のスナップショットではなく最新ドキュメントを確認する
