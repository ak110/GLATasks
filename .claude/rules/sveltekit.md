...
- 背景色の階層構造: ダークモードではgray-950（ヘッダー）→ gray-900（ページ全体）→ gray-800（カード・コンテンツ領域）→ gray-700（ボタン・入力フォーム）の4段階で奥行きを表現する。ライトモードのgray-800（ヘッダー）→ gray-50（ページ全体）→ white（カード）→ gray-100（ボタン）と同じ階層関係を維持する
- テキスト・ボーダーの明暗反転: grayスケールを概ね反転させるが、単純な数値反転ではない。背景色とのコントラスト比（WCAG AA基準4.5:1を目安）を維持するように調整している
