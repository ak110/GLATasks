...
- 品目別カロリーの記録と直近24時間・7日間平均・28日間平均の表示（1日当たりの目標値との比較、摂取可能量の表示、CSVインポート・エクスポート）
...
直近24時間の摂取量と、直近7日間及び28日間の1日当たり平均を、目標値に対する割合とともに確認できる。
直近24時間のカードには、目標値までの残りのkcalを表示する。
目標値を超えている場合は、超過しているkcalを表示する。
各記録は摂取から21時間後までは全量を数え、21時間後から27時間後にかけて数える量が0まで減る。
7日間平均と28日間平均でも同じように、期間の前後3時間をかけて数える量が減る。
kcalと数量は整数で扱う。
